...
네트워크 보안과 시스템 보안은 마치 사회의 SOC와도 같은 개념이다. 보장해주어야만 한다.
...
![what-is-a-tcp-3-way-handshake-process-three-way-handshaking-establishing-connection-6a724e77ba96e241](https://github.com/DevNathan/study_security/assets/142222091/3b221676-ce87-449f-b1b5-bd61131112d5)
	...
	물론 통신 종료를 위해 FIN패킷으로 3-way 방식으로 통신 종료를 할 수 있으나, 서로 상호확인 하여 종료하기 위해서는
...
### 3-3 패스워드 공격
	...
	2) 사전 공격(dictionary attack) 
  			...
  			아이디와 비밀번호 목록을 파일화 시켜서 조합으로 찾아내는 방식
...
3) tcpdump -i [인터페이스] -w [파일이름] -s [패킷 크기] [패킷 타입] port [포트번호]
...
4) wireshark
		...
		와이어샤크는 패킷 스니퍼이며, 간편하고 상세하며 쓰기 쉬운 것이 장점이다. 하지만 프로그램이
		무거운 것이 단점이므로 tcpdump로 패킷을 분석하고 와이어샤크로 상세히 분석하는 형태로 자주 쓰인다.

  		하지만 모든 패킷을 다 스니핑하므로 본인이 원하는 패킷을 필터링하기 위해선 필터링 방법을 어느정도 익힐 필요가 있다.
<img width="771" alt="스크린샷 2023-12-05 110212" src="https://github.com/DevNathan/study_security/assets/142222091/e9f4562f-2eaf-4ce6-aa63-fa5c29ec7fa1">

	현재와 같이 많은 패킷이 존재하게 되면 자신이 원하는 패킷을 찾기 어려워 진다.
<img width="364" alt="스크린샷 2023-12-05 110744" src="https://github.com/DevNathan/study_security/assets/142222091/c52dff2e-d7dd-4372-9a20-36b4cde23f4c">

	하지만 특정패킷을 지정해줘 패킷을 필터링하게 된다면
<img width="778" alt="스크린샷 2023-12-05 110754" src="https://github.com/DevNathan/study_security/assets/142222091/e00cc23b-4d41-4757-84c7-ff091f103b91">

	많은 패킷 중 필터링을 거친 것만 출력 되게 된다.

 다음은 필터링 명령어이다.

		1. IP 주소 필터링
  			ip.addr == 192.168.-.- : 소스IP와 종착지IP가 해당 IP인 경우 조회한다.

			ip.src == 192.168.-.- : 해당 소스IP만 조회한다.
			ip.dst == 192.168.-.- : 해당 종착지IP만 조회한다.

   		2. 포트 필터링
			tcp.port == 80 : 소스포트와 종착지포트가 해당 포트번호일 시 조회한다.

			tcp.srcport == 80 : 해당 소스포트만 조회한다.
   			tcp.dstport == 80 : 해당 종착지포트만 조회한다.

			(udp도 가능)

		3. 논리연산자
			and/&& : 두 필터링 조건식을 연결하여 둘 다 조건에 맞을 시 조회한다.
   			or/|| : 두 필터링 조건식 중 하나만 조회할 수 있으면 조회한다.
			!= : 해당 조건식을 제회하여 조회한다.

		4. 플래그
			tcp.flags.syn == 1 : tcp에 syn패킷이 존재할 시 조회한다.
...
<img width="574" alt="스크린샷 2023-12-05 113644" src="https://github.com/DevNathan/study_security/assets/142222091/30fd399a-5a05-4f6a-87b4-52ffd33ec1dd">
	...
	이러한 결과들은 BackLog에 계속해서 축적되게 되는데
